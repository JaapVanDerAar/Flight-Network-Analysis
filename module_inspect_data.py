...
def inspect_most_flights_airports():
    test = df_merged['source airport'].value_counts()[:10].reset_index()
...
if inspect_option == '1':
    ...
    if columns_dataframe == '1':
        inspect_columns(df_routes)
    elif columns_dataframe == '2':
        inspect_columns(df_airports)
    elif columns_dataframe == '3':
        inspect_columns(df_merged)
    else:
    ...
    if unique_dataframe == '1':
        inspect_unique_values(df_routes)
    elif unique_dataframe == '2':
        inspect_unique_values(df_airports)
    elif unique_dataframe == '3':
        inspect_unique_values(df_merged)
    else:
        print('Sorry, this is not an option, we will use the default setting') 
    ...
    if extra_options == '1':
        ...
        df_merged['country'].value_counts()[0:10].plot.pie()
    elif extra_options == '2':
        print('\nThe 10 biggest airports based on flights: \n')
        inspect_most_flights_airports()
    elif extra_options == '3': 
        print('\nThe 10 biggest airports based on degree: \n')
        hubs_table = comp.find_hubs_in_df(df_merged, 10)
        comp.barplot_hubs(hubs_table)
